...
export class SocketService {
  ...
  setIO(io: SocketIOServer<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>) {
    this.io = io
  }
  ...
  emitConversationUpdate(organizationId: string, conversationId: string, data: {
    lastMessage?: {
      id: string
      content: string
      fromMe: boolean
  ...
  // Emit contact update
  emitContactUpdate(organizationId: string, contactId: string, data: {
    name?: string
    lastSeen?: string
  }) {
    if (!this.io) {
  ...
  emitWhatsAppStatus(organizationId: string, instanceId: string, status: string, qrCode?: string) {
    if (!this.io) {
      console.warn('Socket.IO not initialized')
      return
    }
